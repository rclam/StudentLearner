% Wgn sample 19
% obj: -plot diffStress over Displacement
%      -plot delta(diffStress)/delta(Displacement)
%      -compute machine stiffness from del(dS)/del(Disp) linear portion
%      -document when deviate from linear as a function of seismic cycle
clear
clc
...
%-- import data, set-up
%==========================================================================
load('MTS_Wgn19_Pc_150_Axial.mat');

% new time vector [s]
t_new = 0.12 : 0.10 : 14478;
t     = t_new(1:120405);

% shorten vectors
Displacement = Displacement(1:120405);
diffStress = diffStress(1:120405);
...
%-- noise
n_min   = min(diffStress(1:350));
n_max   = max(diffStress(1:350));
n_range = n_max - n_min;
fprintf('Noise range is: %.2f MPa\n', n_range)

%-- time step
ts = Time(2) - Time(1);
fprintf('dt is: %.2f seconds\n', ts)
...
% legend('data','lsq-fit')
% title(['Wgn 19 (first k removed), r = ',num2str(r_edit(2))])
...
% legend('data','lsq-fit')
% title(['Wgn 19 (first, last k removed), r = ',num2str(r_edit2(2))])
% 
...
% 
% figure(7)
% scatter(drops_edit3, k_edit3, 'b')
% xlabel('\Delta diffStress (MPa)')
% ylabel('Machine Stiffness k (MPa/mm)')
% 
% hold on
% plot(drops_edit3, y_hat_edit3, 'r--')
% legend('data','lsq-fit')
% title(['Wgn 19 (first 2 k removed), r = ',num2str(r_edit3(2))])
